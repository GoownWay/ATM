...
import bean.Card;
import dao.adminDao.DeleteUserDao;
 ...
 *注销用户账号输入界面
 */
public class DeleteUserFrame implements FrameInterface, ActionListener {
	MyFrame frame;
	JPanel panel;
	JLabel label;
	...
	@Override
	public void init() {
		frame = new MyFrame("注销用户");
		panel = new JPanel();

		label = new JLabel("<html><font size=7>请输入待注销的用户账号:</font><br>&nbsp<font size=5>lease enter the account to be cancelled:</font></html>");
		label.setFont(new java.awt.Font("宋体", 0, 35));
		label.setForeground(Color.white);

		userAccount = new MyTextField();// 输入待注销的用户账号
	}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand()=="<html><font size=6>确认</font><br>&nbsp;Comfirm</html>") {
			...
			Card card = delete.queryIsExist(userAccount.getText());//判断账号存在
			if(card != null) {
				UserInfoFrame f = new UserInfoFrame();//存在即跳转到信息界面
				f.show();
				f.listen();
				f.setAccount(card, 4);//传入账号，0=冻结，1=挂失，2\3=解冻解挂，4=注销
				frame.dispose();
			}
		}else {
