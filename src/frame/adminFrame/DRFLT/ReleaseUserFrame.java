 ...
 * 用户解挂
 */
public class ReleaseUserFrame implements FrameInterface, ActionListener {
	MyFrame frame;
	JPanel panel;
	JLabel label;
	...
	@Override
	public void init() {
		frame = new MyFrame("解挂用户");
		panel = new JPanel();

		label = new JLabel("<html><font size=7>请输入待解挂的用户账号:</font><br>&nbsp<font size=5>Please enter the account to be revoked:</font></html>");
		label.setFont(new java.awt.Font("宋体", 0, 35));
		label.setForeground(Color.white);

		userAccount = new MyTextField();// 输入待解挂的用户账号
	}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand()=="<html><font size=6>确认</font><br>&nbsp;Comfirm</html>") {
			...
			if(card != null) {
				...
				f.show();
				f.listen();
				f.setAccount(card, 2);//传入账号以及card，0=冻结，1=挂失，2\3=解冻解挂，4=注销
				frame.dispose();
			}
		}else {
